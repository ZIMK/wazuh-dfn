...
async def test_alerts_watcher_service_file_position_tracking_during_back_pressure(mock_wazuh_service):
    ...
    3. File monitoring resumes from exact position when connection is restored
    4. No alerts are lost or duplicated during the process
    """
    config = WazuhConfig()
    temp_path = create_temp_file()
    config.json_alert_file = temp_path
    ...
    try:
        # Start the service
        task = asyncio.create_task(service.start())
        await asyncio.sleep(0.1)  # Let service initialize

        # Write initial alerts while connected
        initial_alerts = [
        ...
        assert queue_size_during_outage == initial_queue_size, (
            f"Queue size should not increase during outage. "
            f"Before: {initial_queue_size}, During: {queue_size_during_outage}"
        )

        # Verify file position is maintained (not advanced) during outage
        position_during_outage = service.file_monitor.last_complete_position
        LOGGER.info(f"File position during outage: {position_during_outage}")
        assert position_during_outage == position_before_outage, (
        ...
        assert position_after_reconnection > position_before_outage, (
            f"File position should advance after reconnection. "
            f"Before: {position_before_outage}, After: {position_after_reconnection}"
        )

        # Verify alert content integrity - check that we got all expected alerts
        processed_alerts = []
        while not alert_queue.empty():
            alert = await alert_queue.get()
            processed_alerts.append(alert)

        # Extract alert IDs to verify we got all expected alerts
        processed_ids = [alert.get("id") for alert in processed_alerts if "id" in alert]
        expected_ids = [alert["id"] for alert in initial_alerts + outage_alerts]

        # Check that all expected alerts were processed
