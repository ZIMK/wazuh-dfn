...
class TestLogConfigValidators:
    ...
    def test_validate_log_level_with_enum_values(self, level):
        """Test that log levels from the LogLevel enum are accepted."""
        result = LogConfig.validate_log_level(level)
        assert result == level

    @given(st.sampled_from(["debug", "info", "warning", "error", "critical"]))
    def test_validate_log_level_case_sensitivity(self, lowercase_level):
